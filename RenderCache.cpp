...
//extern float gPdfFactor;
//boolean bFirst = TRUE;
SizeI firstSize, preSize;
...
UINT RenderCache::PaintTile(HDC hdc, RectI bounds, DisplayModel *dm, int pageNo,
                            ...
                            bool *renderOutOfDateCue, bool *renderedReplacement, WindowInfo *win)
{
    ...
    if (bmpDC) {
		...
		//**********************************************************************************
		if(pageNo == 1 && win->bSignedTrue != 0 && win->bSignedTrue != -2 && win->mouseAction != MA_SELECTING)
			if((bmpSize.dx >= preSize.dx && bmpSize.dy >= preSize.dy) || win->m_pdfFactor <= 200 ) 
			{
				HBITMAP gBitmap;
				if(win->bSignedTrue == 1)
					gBitmap = LoadBitmap(GetModuleHandle(NULL), MAKEINTRESOURCE(IDB_STAMP_TRUE));
				else
					gBitmap = LoadBitmap(GetModuleHandle(NULL), MAKEINTRESOURCE(IDB_STAMP_FALSE2));

				HDC bmpDC1 = CreateCompatibleDC(hdc);
				
				if (bmpDC1) {

		
					SelectObject(bmpDC1, gBitmap);
					//int size = (int)(16 * win.uiDPIFactor);
					int size = 16;
					int cx = min(bounds.dx, 2 * size), cy = min(bounds.dy, 2 * size);

					//2020-05-22 해상도에 따른 가로세로 폭 조정
					...
					//StretchBlt(bmpDC, 
					//	int(1.0f * 1 * width / 1920 * bmpSize.dx / firstSize.dx),
					//	int(1.0f * 1 * height / 1080 * bmpSize.dy / firstSize.dy), 
					//	int(1.0f * 80 * width / 1920 * bmpSize.dx / firstSize.dx), //해상도에 따른 가로세로 폭 조정 2020-05-22
					//	int(1.0f * 80 * height / 1080 *bmpSize.dy / firstSize.dy),
					//	bmpDC1, 
					//	0, 
					//	0, 
					//	400, 
					...
					if(bmpSize.dx < bmpSize.dy)
					{
						StretchBlt(bmpDC, 
							int(1.0f * 1 * width / 1920 * bmpSize.dx / firstSize.dx),
							int(1.0f * 1 * height / 1080 * bmpSize.dy / firstSize.dy), 
							int(1.0f * 84 * width / 1920 * bmpSize.dx / firstSize.dx), //해상도에 따른 가로세로 폭 조정 2020-05-22
							int(1.0f * 84 * height / 1080 *bmpSize.dy / firstSize.dy),
							bmpDC1, 
							0, 
							0, 
							400, 
						...
						StretchBlt(bmpDC, 
							int(1.0f * 2 * width / 1920 * bmpSize.dx / firstSize.dx),
							int(1.0f * 2 * height / 1080 * bmpSize.dy / firstSize.dy), 
							int(1.0f * 95 * width / 1920 * bmpSize.dx / firstSize.dx), //해상도에 따른 가로세로 폭 조정 2020-05-22
							int(1.0f * 95 * height / 1080 *bmpSize.dy / firstSize.dy),
							bmpDC1, 
							0, 
							0, 
							400, 
        ...
        if (factor != 1.0f){
            ...
            StretchBlt(hdc, bounds.x, bounds.y, bounds.dx, bounds.dy, bmpDC, (int)(xSrc * factor), (int)(ySrc * factor),(int)(bounds.dx * factor), (int)(bounds.dy * factor), SRCAND);

		}
        else{

            BitBlt(hdc, bounds.x, bounds.y, bounds.dx, bounds.dy,bmpDC, xSrc, ySrc, SRCAND);
			//StretchBlt(hdc, bounds.x, bounds.y, bounds.dx, bounds.dy, bmpDC, (int)(xSrc * factor), (int)(ySrc * factor),(int)(bounds.dx * factor), (int)(bounds.dy * factor), SRCAND);
...
UINT RenderCache::Paint(HDC hdc, RectI bounds, DisplayModel *dm, int pageNo,
                        PageInfo *pageInfo, bool *renderOutOfDateCue, WindowInfo *win)
{
    ...
    while (queue.Count() > 0) {
        ...
        UINT renderDelay = PaintTile(hdc, isect, dm, pageNo, tile, tileOnScreen, isTargetRes,
                                     renderOutOfDateCue, isTargetRes ? &neededScaling : NULL, win);
