...
char *ReadAll(const WCHAR *filePath, size_t *fileSizeOut, Allocator *allocator)
{
	...
	if(strFilePath.Find(L"ZeroEnc_") == 0)
	{
		BYTE* pPlain = NULL;
		unsigned int nPlainLen = static_cast<unsigned int>(size64);
		CSeed seed;
		CStringA strUserKey = "1234567890"; //의미없는 암호임. 하지만 암호화, 복호화시에 같은 문자여야 함..키값이기 때문
		seed.SetKey(strUserKey);

		pPlain = seed.Decrypt((LPBYTE)data, nPlainLen);
		Allocator::Free(allocator, data);
		data = (char *)Allocator::Alloc(allocator, nPlainLen + sizeof(WCHAR));
		data[size] = data[nPlainLen + 1] = '\0';
	}
	else
    {
