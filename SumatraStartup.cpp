...
//2020-06-17
#pragma comment(lib,"intechHook2.lib")
extern "C" __declspec(dllimport) BOOL InstallIntechHook(HWND hWnd);
extern "C" __declspec(dllimport) BOOL UnInstallIntechHook(HWND hWnd);
...
static bool RegisterWinClass(HINSTANCE hinst)
{
    ...
    wcex.hIcon  = LoadIcon(hinst, MAKEINTRESOURCE(IDI_CERTPIA));
...
static void GetCommandLineInfo(CommandLineInfo& i)
{
	...
	//200120 로딩시 파라메터 셋팅해줘야 함.
	//CStringW strCmdLine;
	//strCmdLine = "SumatraPDF.exe C:\\서명파일.pdf";//guswogh
	//i.ParseCommandLine(strCmdLine.GetBuffer());
...
static void SetupCrashHandler()
{
    ...
    if (tmpDir)
        symDir.Set(path::Join(tmpDir, L"ZeroView-symbols"));
    else
        symDir.Set(AppGenDataFilename(L"ZeroView-symbols"));
...
//사용자가 요구하는 이미지 파일형태의 CLSID를 구함
//format은 "image/jpeg", "image/bmp", "image/gif", "image/tiff", "image/png" 중 1나
int GetEncoderClsid(const WCHAR* format/*IN*/, CLSID* pClsid/*OUT*/)
{
...
CString getVersion()
{
	...
	if (hRsrc != NULL)
	{
		...
		if (hGlobalMemory != NULL)
		{
			...
			DWORD uLength,langD;
			BOOL retVal;
			retVal = VerQueryValue(pVersionResouece, L"\\VarFileInfo\\Translation", (LPVOID*)&pVersion, (UINT*)&uLength);
			if (retVal && uLength == 4) 
			{
				memcpy(&langD,pVersion,4); 
				rVersion.Format(L"\\StringFileInfo\\%02X%02X%02X%02X\\ProductVersion",
					(langD & 0xff00)>>8,langD & 0xff,(langD & 0xff000000)>>24, 
					(langD & 0xff0000)>>16);     
			}
			else
			{
				rVersion.Format(L"\\StringFileInfo\\%04X04B0\\ProductVersion", GetUserDefaultLangID());
			}

			if( VerQueryValue(pVersionResouece, (LPWSTR)(LPCWSTR)rVersion, (LPVOID*)&pVersion, (UINT *)&uLength) != 0 )
			{
				strVersion.Format(L"%s", pVersion);
			}
		}
...
//프로그램 시작 찾음 //2019-12-26
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
    int retCode = 1;    // by default it's error	

	CStringW strCmdLine = CA2W(lpCmdLine);
	//MessageBox(NULL,strCmdLine,NULL,MB_OK);
	//strCmdLine = "C:\\서명파일.pdf";//guswogh
	//MessageBox(NULL,strCmdLine,NULL,MB_OK);
	//if(strCmdLine.GetLength() == 0)
	//	return retCode;	
	
	//CStringW strFilePath = strCmdLine;
	...
	*/
	...
	//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	{
		...
		LPCWSTR postheader = L"Content-Type: application/x-www-form-urlencoded";
		int port = 80;

		hOpen = InternetOpen(L"SimplePost", INTERNET_OPEN_TYPE_PRECONFIG, 0, 0, 0);
		if(hOpen == NULL)  {
			return -1;
		}
		// "127.0.0.1"대신에 다운받을 화일이 있는 서버주소를 입력 

		hConnect = InternetConnect(hOpen, L"www.certpia.com", (INTERNET_PORT)port, 0, 0, INTERNET_SERVICE_HTTP, 0, 0);
		hReq = HttpOpenRequest(hConnect, L"GET", L"/upfile/ocx/Intech_Launcher/intechPdf.ini", L"HTTP/1.1", 0, 0, INTERNET_FLAG_NO_CACHE_WRITE, 0);
		...
		{     // 다운받는화일의 나머지 크기 얻어오기
			...
			if(dwRead) {
				Data[dwRead] = 0x00;
				//FileWrite(Data, Size);
				strVerData = strVerData + (CString)Data;
			}
		...
		CString updateVer = strVerData.Left(strVerData.Find('$', 0));
		strVerData.Delete(0, strVerData.Find('$', 0) + 1);
		CString updateCode = strVerData.Left(strVerData.Find('$', 0));
		strVerData.Delete(0, strVerData.Find('$', 0) + 1);
		CString updateUrl = strVerData;

	
		//MessageBox(NULL,updateVer,getVersion(),MB_OK);
		if(updateVer.Find(getVersion(),0) == -1){
			if(updateCode == "C") // 하위호환
			{
				if(MessageBox(NULL,L"새로운 버젼이 있습니다. 신규버젼을 설치하시겠습니까?",L"검증뷰어 업데이트안내",MB_YESNO)==IDYES){
					ShellExecute(NULL, L"open", updateUrl, L"", L"", SW_SHOW);
					return 1;
				}
			}
			else if(updateCode == "M") //강제업데이트
			{
				MessageBox(NULL,L"새로운 버젼이 있습니다. 다운로드 페이지에서 신규버젼을 설치 후 사용해 주세요",L"검증뷰어 업데이트안내",MB_OK);
				ShellExecute(NULL, L"open", updateUrl, L"", L"", SW_SHOW);
				return 1;
			}
			else if(updateCode == "X") //업데이트무관
			{
				;
			}
		}else{
	...
	if (isFirstWin) {
        win = CreateAndShowWindowInfo(); //윈도우 생성
        if (!win)
            goto Exit;
    }

	//2020-06-17
	InstallIntechHook(win->hwndFrame);
   ...
   // if (gGlobalPrefs->checkForUpdates && gWindows.Count() > 0)
     //   AutoUpdateCheckAsync(gWindows.At(0)->hwndFrame, true); //업데이트 체크 찾음
	...
	//LoadDocument(args);
    retCode = RunMessageLoop(); //guswogh 여기서 윈도우가 시작됨 찾음 //191226

    CleanUpThumbnailCache(gFileHistory);

	//2020-06-17
	UnInstallIntechHook(win->hwndFrame);
