...
#import "intechVerify.tlb" no_namespace named_guids
...
//#define ABOUT_BG_LOGO_COLOR     RGB(0xCC, 0xCC, 0xCC)
#define ABOUT_BG_LOGO_COLOR     RGB(0xF1, 0xF1, 0xF1)
//#define ABOUT_BG_GRAY_COLOR     RGB(0xCC, 0xCC, 0xCC)
#define ABOUT_BG_GRAY_COLOR     RGB(0xF1, 0xF1, 0xF1)
...
#define RESTRICTIONS_FILE_NAME       L"ZeroViewrestrict.ini"
#define CRASH_DUMP_FILE_NAME         L"ZeroViewcrash.dmp"
...
#else
#define SUMATRA_UPDATE_INFO_URL L"http://www.ZeroView.com/update.txt"
#endif
#endif
...
#else
#define SVN_UPDATE_LINK         L"http://www.ZeroView.com/update"
#endif
#endif
...
static bool LoadDocIntoWindow(LoadArgs& args, PasswordUI *pwdUI, DisplayState *state=NULL)
{
    ...
    */
    if (win->dm) {
        ...
        delete prevModel;
        ScopedMem<WCHAR> title(str::Format(L"%s - %s", path::GetBaseName(args.fileName), SUMATRA_WINDOW_TITLE));
	...
	title.Set(str::Format(L"%s - %s", baseName, SUMATRA_WINDOW_TITLE));
    if (IsUIRightToLeft()) {
		...
		title.Set(str::Format(L"%s - %s", SUMATRA_WINDOW_TITLE, baseName));
    }
    free(docTitle);
    if (needRefresh)
...
//2020-06-03
void MyloadDocument(WindowInfo *win, WCHAR *wFileName)
{
	win->m_bFirst = TRUE;
	LoadArgs args(wFileName, win);
...
{
	//{
	//	LoadArgs args(L"C:\\3333.pdf", win);
	//	args.forceReuse = true;
	//	LoadDocument(args);
	//}
    ...
    if (gGlobalPrefs->showStartPage) 
	{
		...
		/*
		//썸네일을 생성하는 부분인데 여기서는 사용하지 않는다 찾음
...
static WindowInfo* CreateWindowInfo()
{
	...
	///////////////////////////////////////////////
	//수정
    //SetMenu(win->hwndFrame, win->menu); //origin
	SetMenu(win->hwndFrame, NULL); //메뉴를 보이지 않는다
	///////////////////////////////////////////////
    ...
    CreateToolbar(win);  //툴바생성 200129
    //CreateSidebar(win); //2020-06-18
    UpdateFindbox(win);
    if (HasPermission(Perm_DiskAccess) && !gPluginMode)
...
HWND hAlertTxt2;
HWND hAlertTxt3;
int CALLBACK MyAlertDialogProc(HWND ah_dlg, UINT a_message_id,  WPARAM wParam, LPARAM lParam) 
{ 
	...
	//	if (LOWORD(wParam) == IDOK || LOWORD(wParam) == IDC_STC_TEXT3){ 

	//		DeleteObject(hBrush);
	//		EndDialog(ah_dlg, LOWORD(wParam)); 
	//		//DestroyWindow(hAlertDlg);
	//		//hAlertDlg = NULL;
	//		return TRUE; 
	//	}
	//} 

	if(a_message_id == WM_LBUTTONDOWN){   

		DeleteObject(hBrush);
		EndDialog(ah_dlg, LOWORD(wParam)); 
		return TRUE; 

	}else if(a_message_id == WM_MOUSEMOVE){
		SetClassLong(ah_dlg, GCL_HCURSOR, (LONG)LoadCursor(NULL, IDC_HAND));
	...
	case WM_INITDIALOG:
		{
			hAlertTxt1 = GetDlgItem(ah_dlg,IDC_STC_TEXT1);
			hAlertTxt2 = GetDlgItem(ah_dlg,IDC_STC_TEXT2);
			hAlertTxt3 = GetDlgItem(ah_dlg,IDC_STC_TEXT3);


			
			//HDC hdc;
			//PAINTSTRUCT ps;
			HFONT font1, font2, font3;

			
			//hdc=BeginPaint(ah_dlg, &ps);
			font1=CreateFont(16,0,0,0,FW_BOLD,0,0,0,HANGEUL_CHARSET,0,0,0,0,L"돋움");
			font2=CreateFont(13,0,0,0,0,0,0,0,HANGEUL_CHARSET,0,0,0,0,L"돋움");
			font3=CreateFont(16,0,0,0,FW_BOLD,0,TRUE,0,HANGEUL_CHARSET,0,0,0,0,L"돋움");
			//oldfont=(HFONT)SelectObject(hdc,font);
			

			SendMessage(hAlertTxt1, WM_SETFONT, (WPARAM)font1,FALSE);
			SendMessage(hAlertTxt2, WM_SETFONT, (WPARAM)font2,FALSE);
			SendMessage(hAlertTxt3, WM_SETFONT, (WPARAM)font3,FALSE);
			//SelectObject(hdc,oldfont);
			//DeleteObject(font);
			//EndPaint(ah_dlg, &ps);

			WindowInfo *win = FindWindowInfoByHwnd(ah_dlg);
			if(win->bSignedTrue == 1){
				hBrush = CreateSolidBrush(RGB(0,47,102));
				SetDlgItemText(ah_dlg,IDC_STC_TEXT1,L"이 문서는 서명 이후 변경되지 않은 진본 문서입니다.");
				SetDlgItemText(ah_dlg,IDC_STC_TEXT2,L"자세한 내용은 서명확인필 마크를 클릭하여 확인하십시오.");
				SetDlgItemText(ah_dlg,IDC_STC_TEXT3,L"확인");
			}else if (win->bSignedTrue == -1){
				hBrush = CreateSolidBrush(RGB(255,80,0));
				SetDlgItemText(ah_dlg,IDC_STC_TEXT1,L"이 문서는 서명 이후 변경된 변조 문서입니다.");
				SetDlgItemText(ah_dlg,IDC_STC_TEXT2,L"자세한 내용은 서명확인필 마크를 클릭하여 확인하십시오.");
				SetDlgItemText(ah_dlg,IDC_STC_TEXT3,L"확인");
			}else if (win->bSignedTrue == -2){
				hBrush = CreateSolidBrush(RGB(255,80,0));
				SetDlgItemText(ah_dlg,IDC_STC_TEXT1,L"이 문서는 이전 서명모듈로 생성되어 진위확인 여부를 확인할 수 없습니다.");
				SetDlgItemText(ah_dlg,IDC_STC_TEXT2,L"이 문서를 검증하려면 www.certpia.com/down 에서 프로그램을 내려받아 확인하십시요.");
				SetDlgItemText(ah_dlg,IDC_STC_TEXT3,L"확인");
			}else{
				hBrush = CreateSolidBrush(RGB(255,80,0));
				SetDlgItemText(ah_dlg,IDC_STC_TEXT1,L"이 문서는 서명이 없어 진위확인 검증이 불가합니다.");
				SetDlgItemText(ah_dlg,IDC_STC_TEXT2,L"");
				SetDlgItemText(ah_dlg,IDC_STC_TEXT3,L"확인");
			}
			

			RECT rect;
			HWND parent = GetParent(ah_dlg);
			GetWindowRect(parent,&rect);
			RECT rect2;
			GetClientRect(ah_dlg, &rect2);
			
			SetWindowPos(ah_dlg, NULL, rect.left + 7 ,rect.top + (rect.bottom - rect.top)/2 - (rect2.bottom - rect2.top)/2 ,rect.right - rect.left - 14,rect2.bottom-rect2.top, NULL);
			RECT rectText1,rectText3;
			GetClientRect(ah_dlg, &rect2);
			GetClientRect(hAlertTxt1, &rectText1);
			//SetWindowPos(hAlertTxt1,NULL,rect2.left + (rect2.right-rect2.left)/2  - (rectText1.right - rectText1.left)/2 , rect2.top, rectText1.right - rectText1.left, rectText1.bottom - rectText1.top, NULL);

			if(win->bSignedTrue == 0){// no signature
				SetWindowPos(hAlertTxt1,NULL,(rect2.right-rect2.left)/2  - (rectText1.right - rectText1.left)/2 , (rect2.bottom-rect2.top)/2  - (rectText1.bottom - rectText1.top)/2, rectText1.right - rectText1.left, rectText1.bottom - rectText1.top, NULL);
				SetWindowPos(hAlertTxt2,NULL,(rect2.right-rect2.left)/2  - (rectText1.right - rectText1.left)/2 , (rect2.bottom - rect2.top)/2 , rectText1.right - rectText1.left, rectText1.bottom - rectText1.top, NULL);
				GetClientRect(hAlertTxt1, &rectText1);
				GetClientRect(hAlertTxt3, &rectText3);
				SetWindowPos(hAlertTxt3,NULL,(rect2.right-rect2.left)/2  - (rectText1.right - rectText1.left)/2 + rectText1.right - rectText1.left ,  (rect2.bottom-rect2.top)/2  - (rectText3.bottom - rectText3.top)/2 , rectText3.right - rectText3.left, rectText3.bottom - rectText3.top, NULL);
			}else{
				SetWindowPos(hAlertTxt1,NULL,(rect2.right-rect2.left)/2  - (rectText1.right - rectText1.left)/2 , 0, rectText1.right - rectText1.left, rectText1.bottom - rectText1.top, NULL);
				SetWindowPos(hAlertTxt2,NULL,(rect2.right-rect2.left)/2  - (rectText1.right - rectText1.left)/2 , (rect2.bottom - rect2.top)/2 , rectText1.right - rectText1.left, rectText1.bottom - rectText1.top, NULL);
				GetClientRect(hAlertTxt1, &rectText1);
				GetClientRect(hAlertTxt3, &rectText3);
				SetWindowPos(hAlertTxt3,NULL,(rect2.right-rect2.left)/2  - (rectText1.right - rectText1.left)/2 + rectText1.right - rectText1.left , 0 , rectText3.right - rectText3.left, rectText3.bottom - rectText3.top, NULL);
			}
			...
			//HBITMAP hbit;
			//if(win->bSignedTrue == 1){
			//	hbit = ::LoadBitmap(GetModuleHandle(NULL), MAKEINTRESOURCE( IDB_INFO_TRUE));
			//}else if(win->bSignedTrue == -1){
			//	hbit = ::LoadBitmap(GetModuleHandle(NULL), MAKEINTRESOURCE( IDB_INFO_FALSE));
			//}else{
			//	hbit = ::LoadBitmap(GetModuleHandle(NULL), MAKEINTRESOURCE( IDB_INFO_ETC));
			//}

			//	
			//HWND hItem = GetDlgItem(ah_dlg, IDC_STC_INFO);
			//SendMessage(hItem, STM_SETIMAGE, IMAGE_BITMAP, (LPARAM)hbit);
	...
	case WM_CTLCOLORSTATIC:
		{
			...
			if((HWND)lParam == hAlertTxt1 || (HWND)lParam == hAlertTxt2  || (HWND)lParam == hAlertTxt3)
			{
				WindowInfo *win = FindWindowInfoByHwnd(ah_dlg);
				if(win->bSignedTrue == 1){
					SetTextColor((HDC)wParam, RGB(255,255,255));
					SetBkColor ((HDC)wParam, RGB(0,47,102));
					...
					SetTextColor((HDC)wParam, RGB(255,255,255));
					SetBkColor ((HDC)wParam, RGB(255,80,0));
...
static WindowInfo* LoadDocumentOld(LoadArgs& args)
{
	...
	/*
	//읽었던 파일을 미리보기 하도록 해주는 부분인데 여기서는 사용하지 않는다 찾음
	...
	*/
	...
	if(loaded){
		...
		HRESULT hr = CoCreateInstance(CLSID_CallClassImp, NULL, CLSCTX_INPROC_SERVER, IID_ICallClass, reinterpret_cast<void**>(&csharp));
		...
		if (SUCCEEDED(hr))
		{
			...
			//intechsvc$(주)아이앤텍(박동기)$2020-05-11 오후 5:52:28$SHA1withRSA$C=KR,O=TradeSign,OU=AccreditedCA,CN=TradeSignCA3$1506136846$2019-11-01 오전 3:36:21$2020-11-06 오전 6:41:38

				
					msg = csharp->verifyPdfSignature( args.fileName ,&lmsg );
				

					CString cmsg = (CString)msg;
					CString sCode = "";
					sCode = cmsg.Left(cmsg.Find('$', 0));
					if(sCode == "intechsvc"){
						cmsg.Delete(0, cmsg.Find('$', 0) + 1);
				
					}

					CString str;
					//gSignatureInfo = cmsg;
					win->m_signatureInfo = cmsg;
			

					if(lmsg == 1 && sCode == "intechsvc"){
						win->bSignedTrue = 1;
		
						//MessageBox(NULL, L"이 문서는 서명이후 변경되지 않았습니다.", L"진본", MB_ICONINFORMATION);
						...
						win->bSignedTrue = -1;
						//MessageBox(NULL, L"이 문서는 서명이후 변경되었습니다.", L"위조", MB_ICONINFORMATION);
					...
					DialogBox(NULL, MAKEINTRESOURCE(IDD_DIALOG_INFO2),  win->hwndFrame, MyAlertDialogProc);
					//if( !IsWindow(hAlertDlg)){
					//	hAlertDlg = CreateDialog(NULL, MAKEINTRESOURCE(IDD_DIALOG_INFO2),  win->hwndCanvas, MyAlertDialogProc);
					//	ShowWindow(hAlertDlg,SW_SHOW);
					//}
...
static DWORD ShowAutoUpdateDialog(HWND hParent, HttpReq *ctx, bool silent)
{
    ...
    if (CompareVersion(verTxt, UPDATE_CHECK_VER) <= 0) {
        ...
        if (!silent)
            MessageBoxWarning(hParent, _TR("You have the latest version."), _TR("ZeroView Update"));
        return 0;
    }
...
static void ProcessAutoUpdateCheckResult(HWND hwnd, HttpReq *req, bool autoCheck)
{
    ...
    MessageBoxWarning(hwnd, msg, _TR("ZeroView Update"));
Exit:
    delete req;
}
...
static void DrawDocument(WindowInfo& win, HDC hdc, RECT *rcArea)
{
    ...
    for (int pageNo = 1; pageNo <= dm->PageCount(); ++pageNo) {
        ...
        if (!(dm->engine && dm->engine->IsImageCollection()))
            PaintPageFrameAndShadow(hdc, bounds, pageInfo->pageOnScreen, win.presentation);  //외곽선 그리기
        ...
        if (!DoCachePageRendering(&win, pageNo)) {
            ...
            renderDelay = gRenderCache.Paint(hdc, bounds, dm, pageNo, pageInfo, &renderOutOfDateCue, &win);
        ...
        if (bmpDC) {
            ...
            StretchBlt(hdc, 
				bounds.x + bounds.dx - min((cx + size) / 2, cx),
                bounds.y + max((cy - size) / 2, 0), 
				min(cx, size), 
				min(cy, size),
                bmpDC, 
				0, 
...
static void OnMouseMove(WindowInfo& win, int x, int y, WPARAM flags)
{
	...
	//2020-06-15 서명스탬프 마우스 오버기능 추가
	PageElement *pageEl = win.dm->GetElementAtPos(PointI(x, y));	
	if (pageEl && pageEl->GetType() == Element_Image) {
		//SetClassLong(win.hwndFrame , GCL_HCURSOR, (LONG)LoadCursor(NULL, IDC_HAND));
...
{ 
	...
	if(a_message_id == WM_COMMAND){   
		// '확인(IDOK)' 또는 '취소(IDCANCEL)' 버튼이 눌러진 경우 
		if (LOWORD(wParam) == IDC_BTN_CONFIRM){ 
			// 대화 상자를 종료하는 함수, 어떤 버튼에 의해서 종료되었는지  
			// 확인할 수 있도록 두 번째 인자에 버튼 ID 값을 넣는다. 
			EndDialog(ah_dlg, LOWORD(wParam)); 
			return 1; 
		}else if(LOWORD(wParam) == IDC_BTN_VIEW)  {
			//각창에 대응2020-06-02
			WindowInfo *win = FindWindowInfoByHwnd(ah_dlg);
			CString sigInfo = win->m_signatureInfo;
			//CString sigInfo = gSignatureInfo;
			CString sFileName = sigInfo.Left(sigInfo.Find('$', 0));

			
			csharp->viewCertificate(sFileName.AllocSysString());
	...
	case WM_INITDIALOG:
		{
			...
			hTabCtrl = GetDlgItem(ah_dlg, IDC_TAB1);
			...
			//스탬프이미지
			HBITMAP hbit;
			hbit = ::LoadBitmap(GetModuleHandle(NULL), MAKEINTRESOURCE( IDB_TRUEMARK));
			HWND hItem = GetDlgItem(ah_dlg, IDC_STC_PIC);
			SendMessage(hItem, STM_SETIMAGE, IMAGE_BITMAP, (LPARAM)hbit);


			hStcTitle1 = GetDlgItem(ah_dlg,IDC_STC_TITLE1);
			hStcTitle2 = GetDlgItem(ah_dlg,IDC_STC_TITLE2);
			hStcTitle3 = GetDlgItem(ah_dlg,IDC_STC_TITLE3);
			hStcTop1 = GetDlgItem(ah_dlg,IDC_STC_TOP1);
			hStcTop2 = GetDlgItem(ah_dlg,IDC_STC_TOP2);
			hStatic1 = GetDlgItem(ah_dlg, IDC_STC1);
			hStatic2 = GetDlgItem(ah_dlg, IDC_STC2);
			hStatic3 = GetDlgItem(ah_dlg, IDC_STC3);
			hStcTxt1 = GetDlgItem(ah_dlg,IDC_STATIC_TXT1);
			hStcTxt2 = GetDlgItem(ah_dlg,IDC_STATIC_TXT2);
			hStcTxt3 = GetDlgItem(ah_dlg,IDC_STATIC_TXT3);
			hStcTxt4 = GetDlgItem(ah_dlg,IDC_STATIC_TXT4);


			h2StcTxt1 = GetDlgItem(ah_dlg,IDC_STC2_TXT1);
			h2StcTxt2 = GetDlgItem(ah_dlg,IDC_STC2_TXT2);
			h2StcTxt3 = GetDlgItem(ah_dlg,IDC_STC2_TXT3);
			h2StcTxt4 = GetDlgItem(ah_dlg,IDC_STC2_TXT4);
			h2StcTxt5 = GetDlgItem(ah_dlg,IDC_STC2_TXT5);
			h2StcTxt6 = GetDlgItem(ah_dlg,IDC_STC2_TXT6);
			h2StcInfo1 = GetDlgItem(ah_dlg,IDC_STC2_INFO1);
			h2StcInfo2 = GetDlgItem(ah_dlg,IDC_STC2_INFO2);
			h2StcInfo3 = GetDlgItem(ah_dlg,IDC_STC2_INFO3);
			h2StcInfo4 = GetDlgItem(ah_dlg,IDC_STC2_INFO4);
			h2StcInfo5 = GetDlgItem(ah_dlg,IDC_STC2_INFO5);
			h2BtnView = GetDlgItem(ah_dlg,IDC_BTN_VIEW);
			...
			ShowWindow(hStcTxt1, SW_SHOW);
			ShowWindow(hStcTxt2, SW_SHOW);
			ShowWindow(hStcTxt3, SW_SHOW);
			ShowWindow(hStcTxt4, SW_SHOW);


			ShowWindow(h2StcTxt1, SW_HIDE);
			ShowWindow(h2StcTxt2, SW_HIDE);
			ShowWindow(h2StcTxt3, SW_HIDE);
			ShowWindow(h2StcTxt4, SW_HIDE);
			ShowWindow(h2StcTxt5, SW_HIDE);
			ShowWindow(h2StcTxt6, SW_HIDE);
			ShowWindow(h2StcInfo1, SW_HIDE);
			ShowWindow(h2StcInfo2, SW_HIDE);
			ShowWindow(h2StcInfo3, SW_HIDE);
			ShowWindow(h2StcInfo4, SW_HIDE);
			ShowWindow(h2StcInfo5, SW_HIDE);
			ShowWindow(h2BtnView, SW_HIDE);


			
			//(주)아이앤텍(박동기)$2020-05-11 오후 5:52:28$SHA1withRSA$C=KR,O=TradeSign,OU=AccreditedCA,CN=TradeSignCA3$1506136846$2019-11-01 오전 3:36:21$2020-11-06 오전 6:41:38
			
			WindowInfo *win = FindWindowInfoByHwnd(ah_dlg);
			CString sigInfo = win->m_signatureInfo;
			//CString sigInfo = gSignatureInfo;
			sigInfo.Delete(0, sigInfo.Find('$', 0) + 1);


			SetWindowText(hStcTop1,L"이 문서는 서명 이후 데이터가 변경되지 않은 진본임을 확인합니다.");
			CString data1 = sigInfo.Left(sigInfo.Find('$', 0));
			
			//서명자, 발급자
			SetWindowText(hStatic1, data1);
			SetWindowText(h2StcInfo1, data1);
			sigInfo.Delete(0, sigInfo.Find('$', 0) + 1);

			//서명시각
			CString data2 = sigInfo.Left(sigInfo.Find('$', 0));
			SetWindowText(hStatic3, data2);
			sigInfo.Delete(0, sigInfo.Find('$', 0) + 1);

			SetWindowText(hStatic2, L"증명서 진위확인 서비스");

			//서명해시알고리즘
			CString data3 = sigInfo.Left(sigInfo.Find('$', 0));
			SetWindowText(hStcTop2, data3);
			sigInfo.Delete(0, sigInfo.Find('$', 0) + 1);

			//발급기관
			//C=KR,O=TradeSign,OU=AccreditedCA,CN=TradeSignCA3
			CString data4 = sigInfo.Left(sigInfo.Find('$', 0));
			data4.Delete(0,data4.Find(L"O=", 0)+2);
			CString data5 = data4.Left(data4.Find(',',0));
			SetWindowText(h2StcInfo2, data5);
			sigInfo.Delete(0, sigInfo.Find('$', 0) + 1);

			//일련번호
			CString data6 = sigInfo.Left(sigInfo.Find('$', 0));
			SetWindowText(h2StcInfo3,data6);
			sigInfo.Delete(0, sigInfo.Find('$', 0) + 1);

			//시작일
			CString data7 = sigInfo.Left(sigInfo.Find('$', 0));
			SetWindowText(h2StcInfo4,data7);
			sigInfo.Delete(0, sigInfo.Find('$', 0) + 1);

			//만료일
			CString data8 = sigInfo;
			SetWindowText(h2StcInfo5,data8);
	...
	case WM_NOTIFY:
		{
			...
			if(wParam == IDC_TAB1){
				...
				switch(nmhdr->code){
				case TCN_SELCHANGE:
					iTab = TabCtrl_GetCurFocus( hTabCtrl );
					if(iTab == 0){
						ShowWindow(hStatic1, SW_SHOW);
						ShowWindow(hStatic2, SW_SHOW);
						ShowWindow(hStatic3, SW_SHOW);
						...
						ShowWindow(hStcTxt1, SW_HIDE);
						ShowWindow(hStcTxt2, SW_HIDE);
						ShowWindow(hStcTxt3, SW_HIDE);
						ShowWindow(hStcTxt4, SW_HIDE);




						ShowWindow(h2StcTxt1, SW_SHOW);
						ShowWindow(h2StcTxt2, SW_SHOW);
						ShowWindow(h2StcTxt3, SW_SHOW);
						ShowWindow(h2StcTxt4, SW_SHOW);
						ShowWindow(h2StcTxt5, SW_SHOW);
						ShowWindow(h2StcTxt6, SW_SHOW);
						ShowWindow(h2StcInfo1, SW_SHOW);
						ShowWindow(h2StcInfo2, SW_SHOW);
						ShowWindow(h2StcInfo3, SW_SHOW);
						ShowWindow(h2StcInfo4, SW_SHOW);
						ShowWindow(h2StcInfo5, SW_SHOW);
						ShowWindow(h2BtnView, SW_SHOW);
					}
				}
			}
			break;
	...
	case WM_CTLCOLORSTATIC:
		{
			...
			if((HWND)lParam != hStcTitle1 && (HWND)lParam != hStcTitle2 && (HWND)lParam != hStcTitle3)
			{
...
static void OnMouseLeftButtonDown(WindowInfo& win, int x, int y, WPARAM key)
{
	...
	if (pageEl && pageEl->GetType() == Element_Link)
		...
		//if(gSignatureInfo == "NONE")
		if(win.m_signatureInfo == "NONE")
		{
			MessageBox(win.hwndFrame, L"서명정보는 진본 문서만 확인할 수 있습니다.",L"알림", MB_OK);
		}
		else
			//2020-05-28
			DialogBox(NULL, MAKEINTRESOURCE(IDD_DIALOG_SIGNATURE),  win.hwndFrame, MyMenuDialogProc); 
...
void OnMenuOpen(const SumatraWindow& win)
{
    ...
    const struct {
        ...
        { NULL, L"*.tif;*.tiff",true }, // multi-page images 
        { NULL,   L"*.pdb;*.tcr", gGlobalPrefs->ebookUI.useFixedPageUI }, // further ebooks
    ...
    if (*(fileName - 1)) {//단일 파일
		...
		LoadArgs args(fileName, win.AsWindowInfo());
		//ReloadDocument(args.win);
		MyloadDocument(args.win,fileName);
        return;
    }

    while (*fileName) //여러파일 선택했을 경우
	{
...
static LRESULT CanvasOnMouseWheel(WindowInfo& win, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    if ((LOWORD(wParam) & MK_CONTROL) || IsCtrlPressed() || (LOWORD(wParam) & MK_RBUTTON)) {
		//2020-06-19 컨트롤 휠 확대시 진본텍스트 오류 일단 주석처리
        //POINT pt;
        //GetCursorPosInHwnd(win.hwndCanvas, pt);
    ...
    switch (msg)
    {
        ...
        case WM_MOVE:
            if (win) {
				...
				//if(hAlertDlg){
				//	RECT rect1, rect2;
				//	GetWindowRect(win->hwndFrame, &rect1);
				//	GetClientRect(hAlertDlg, &rect2);
				//	SetWindowPos(hAlertDlg, NULL, rect1.left, rect1.top + 500, rect2.right-rect2.left, rect2.top-rect2.bottom, SWP_NOSIZE);
				//}
            }
            break;
        ...
        case WM_COMMAND:
            return FrameOnCommand(win, hwnd, msg, wParam, lParam); //툴바메뉴쪽 이벤트 처리 200129
...
void CrashHandlerMessage()
{
    ...
    //if (HasPermission(Perm_DiskAccess)) {
    //    int res = MessageBox(NULL, _TR("Sorry, that shouldn't have happened!\n\nPlease press 'Cancel', if you want to help us fix the cause of this crash."), _TR("ZeroView crashed"), MB_ICONERROR | MB_OKCANCEL | MbRtlReadingMaybe());
    //    if (IDCANCEL == res)
