...
public:
	...
	void SetKey(LPCSTR lpszUserKey);
	void SetKey(LPBYTE lpUserKey) { SeedEncRoundKey(lpUserKey); }
	DWORD* GetRoundKey() { return m_dwRoundKey; }
	void Reserve(int nLen) { m_nMinDataLen = nLen; }

	LPBYTE Encrypt(const LPBYTE lpData, int& nDataLen);
	LPBYTE Decrypt(const LPBYTE lpData, int nDataLen);

	void SeedEncrypt(LPBYTE pbData);
	void SeedDecrypt(LPBYTE pbData);

protected:
	int CalcDataLen(int nDataLen) { return (nDataLen+(SeedBlockSize-1))/SeedBlockSize*SeedBlockSize; }
	void SeedEncRoundKey(LPBYTE pbUserKey);

private:
	LPBYTE	m_pData;
	int		m_nDataLen;
	int		m_nMinDataLen;
	DWORD	m_dwRoundKey[NoRoundKeys];
	const DWORD	*SS0;
	const DWORD	*SS1;
	const DWORD	*SS2;
	const DWORD	*SS3;
...
CSeed::CSeed(void) : SS0(_SS0), SS1(_SS1), SS2(_SS2), SS3(_SS3), m_pData(NULL), m_nDataLen(0), m_nMinDataLen(64)
{
	
	ZeroMemory(m_dwRoundKey, sizeof(m_dwRoundKey));
}
...
void CSeed::SetKey(LPCSTR lpszUserKey)
{
	BYTE byteKey[16] = {0,};
	memcpy(byteKey, lpszUserKey, strlen(lpszUserKey));
	SetKey(byteKey);
}

LPBYTE CSeed::Decrypt(const LPBYTE lpData, int nDataLen)
{
	int nBlockCount = nDataLen/SeedBlockSize;
	int nLen = CalcDataLen(nDataLen);
	if (m_nDataLen < nLen) 
	{
		if (m_pData)
			delete [] m_pData;
		size_t nDataSize = max(m_nMinDataLen, nLen);
		m_pData = new BYTE [nDataSize];
		m_nDataLen = (int)nDataSize;
	}
	LPBYTE pDest = m_pData, pSrc = lpData;
	if (pDest != pSrc) 
	{
		for (int i = 0; i < nBlockCount; ++i) 
		{
			CopyMemory(pDest, pSrc, SeedBlockSize);
			SeedDecrypt(pDest);
			pDest += SeedBlockSize;
			pSrc += SeedBlockSize;
		}
...
// Block Encryption
void CSeed::SeedEncrypt(LPBYTE pbData)
{
	DWORD L0, L1, R0, R1, T0, T1, *K = m_dwRoundKey;
...
void CSeed::SeedDecrypt(LPBYTE pbData)
{
	DWORD L0, L1, R0, R1, T0, T1, *K = m_dwRoundKey;
...
void CSeed::SeedEncRoundKey(LPBYTE pbUserKey)
{
	DWORD A, B, C, D, T0, T1, *K = m_dwRoundKey;
