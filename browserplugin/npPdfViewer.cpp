...
#define PREFS_FILE_NAME L"ZeroView-settings.txt"
...
#ifndef _WIN64
const WCHAR *g_lpRegKey = L"Software\\MozillaPlugins\\@mozilla.zeniko.ch/ZeroView_Browser_Plugin";
#else
const WCHAR *g_lpRegKey = L"Software\\MozillaPlugins\\@mozilla.zeniko.ch/ZeroView_Browser_Plugin_x64";
#endif
...
DLLEXPORT STDAPI DllRegisterServer(VOID)
{
    ...
    if (!WriteRegStr(hkey, g_lpRegKey, L"Description", L"ZeroView Browser Plugin") ||
        !WriteRegStr(hkey, g_lpRegKey, L"Path", szPath) ||
        !WriteRegStr(hkey, g_lpRegKey, L"Version", CURR_VERSION_STR) ||
        !WriteRegStr(hkey, g_lpRegKey, L"ProductName", L"ZeroView Browser Plugin"))
    {
        return E_UNEXPECTED;
    }
...
bool GetExePath(WCHAR *lpPath, size_t len)
{
    ...
    str::BufSet((WCHAR *)path::GetBaseName(lpPath), len - 2 - (path::GetBaseName(lpPath) - lpPath), L"ZeroView.exe");
    if (file::Exists(lpPath))
        return true;
    
    *lpPath = '\0';
    // Try to get the path from the registry (set e.g. when making the default PDF viewer)
    ScopedMem<WCHAR> path(ReadRegStr(HKEY_CURRENT_USER, L"Software\\Classes\\ZeroView\\Shell\\Open\\Command", NULL));
    if (!path)
        return false;
...
void SelectTranslation(const WCHAR *exePath=NULL)
{
    ...
    if (!file::Exists(path)) {
        ...
        path.Set(path::Join(path, L"ZeroView\\" PREFS_FILE_NAME));
    }
    if (!file::Exists(path))
        return;
...
NPError NP_LOADDS NPP_New(NPMIMEType pluginType, NPP instance, uint16_t mode, int16_t argc, char* argn[], char* argv[], NPSavedData* saved)
{
    ...
    if (ok)
        data->message = _TR("Opening document in ZeroView...");
    else
        data->message = _TR("Error: ZeroView hasn't been found!");
...
void LaunchWithSumatra(InstanceData *data, const char *url_utf8)
{
    ...
    if (!data->hProcess)
    {
        plogf("sp: NPP_StreamAsFile() error: couldn't run ZeroView!");
        data->message = _TR("Error: Couldn't run ZeroView!");
    }
}
